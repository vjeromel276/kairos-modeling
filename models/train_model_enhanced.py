...
outputs predictions (with actuals), and computes SHAP values for LightGBM and XGBoost.
Includes SHAP-based feature scaling and tagging for output separation.
"""
...
parser.add_argument('--tag', type=str, default='', help='Optional tag to append to model outputs')
...
X_train, y_train = df_train[features].copy(), df_train[targets]
X_val, y_val     = df_val[features].copy(), df_val[targets]

# Apply optional SHAP-based scaling
if feature_scaling:
    print("⚖️  Scaling features based on SHAP weights")
    for col, scale in feature_scaling.items():
...
model_name = f"{model_type}_{'_'.join(targets)}_{args.year}{TAG}.pkl"
pred_name = f"predictions_{model_type}_{args.year}{TAG}.csv"
metrics_name = f"metrics_{model_type}_{args.year}{TAG}.csv"
...
if model_type in ['lgbm', 'xgb']:
    ...
    for i, est in enumerate(model.estimators_):
        ...
        shap_file = f"{OUTPUT_DIR}/shap_{model_type}_{targets[i]}_{args.year}{TAG}.csv"
