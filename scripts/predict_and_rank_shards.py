...
Loads trained model (e.g., mh_lgbm_126.pkl), streams through X shards,
predicts y_hat for each ticker, and writes predictions to disk.

Each output:
  mh_126_<TICKER>_pred.parquet
"""
...
def main(window: int, model_path: str, shard_dir: str):
    ...
    for x_path in tqdm(x_files, desc="Predicting shards"):
        ...
        y_path = os.path.join(shard_dir, f"mh_{window}_{ticker}_pred.parquet")
        ...
        try:
            ...
            df_pred.to_parquet(y_path, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--window", type=int, required=True, help="Input window size (e.g. 126)")
    parser.add_argument("--model", type=str, required=True, help="Trained model path (e.g. mh_lgbm_126.pkl)")
